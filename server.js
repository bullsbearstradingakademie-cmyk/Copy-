import express from "express"; import cors from "cors"; import helmet from "helmet"; import morgan from "morgan"; import Database from "better-sqlite3";
const app = express();
...
const rows = db.prepare("SELECT id,copy_id,name,email,blocked,created_at FROM customers ORDER BY id DESC").all();
res.json(rows);
});
...
db.prepare("INSERT INTO events(copy_id,seq,payload) VALUES(?,?,?)").run(copy_id, Number(seq), JSON.stringify(rest));
res.json({ok:true});
});
app.get("/events", requireCustomer, (req,res)=>{
const copy_id = req.headers["x-copy-id"] || req.query.copy_id;
const since = Number(req.query.since || 0);
const rows = db.prepare("SELECT seq,payload,created_at FROM events WHERE copy_id=? AND seq>=? ORDER BY seq ASC").all(copy_id, since);
