...
// Tabellen als SQL-Strings anlegen
...
if (!token || !copyId)
return res.status(400).json({ error: "missing token or copy_id" });
const row = db
.prepare("SELECT * FROM customers WHERE copy_id=? AND token=?")
.get(copyId, token);
...
db.prepare("INSERT INTO customers(copy_id,token,name,email) VALUES(?,?,?,?)")
.run(copy_id, token, name, email);
...
db.prepare("UPDATE customers SET blocked=1 WHERE copy_id=?")
.run(req.params.copy_id);
res.json({ ok: true });
});

app.post("/admin/customers/:copy_id/reset", requireAdmin, (req, res) => {
const token = genToken();
db.prepare("UPDATE customers SET token=? WHERE copy_id=?")
.run(token, req.params.copy_id);
...
db.prepare("INSERT INTO events(copy_id,seq,payload) VALUES(?,?,?)")
.run(copy_id, Number(seq), JSON.stringify(rest));
res.json({ ok: true });
});
